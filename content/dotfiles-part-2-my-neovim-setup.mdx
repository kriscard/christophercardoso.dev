...
<Code type="keys">Space+xx</Code> shows all diagnostics across your workspace. Jump between errors, filter by severity, see them in context. <Code type="keys">Space+cl</Code> shows LSP references in a right-side panel—definition, references, and implementations all in one view.
...
<Code type="keys">Space+ri</Code> adds missing imports automatically. It filters LSP code actions to only those that add/update imports, then applies the first match. One keypress instead of navigating through code action menus.
