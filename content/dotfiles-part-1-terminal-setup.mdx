---
title: "Dotfiles Part 1: Terminal, Tmux & Modern CLI Tools"
date: 2026-01-12
tag: [Dotfiles, Terminal, DevTools, Productivity]
summary: From Ghostty and Zsh to Tmux and GNU Stow—here's how I built a terminal setup that sparks joy.
...
Then something happens, one day I saw Thdxr streaming on Twitch when he was still coding on STT if I'm not mistaken. I saw his neovim setup and his terminal and how cool the UI was and how quick he moved around neovim and its terminal. I was like "Woaw I really want the same setup".
...
Let's start where everything begins, my terminal. I use [Ghostty](https://ghostty.org/), a GPU-accelerated terminal built by [Mitchell Hashimoto](https://x.com/mitchellh) (one of HashiCorp's co-founders). I've tried them all—Kitty, Alacritty, WezTerm—but Ghostty just feels right on macOS. It's really fast, and the config is just a plain text file which makes everything so simple.
...
This approach is way faster than Oh My Zsh for my shell to load. The modular structure also makes it super easy to add or remove things—just edit one file instead of hunting through Oh My Zsh's maze of directories.
...
The difference isn't subtle. Once you try `fd`, regular `find` feels broken. `bat` makes reading code in the terminal actually pleasant. And `zoxide`—once you've used it, you'll wonder how you ever navigated without it.
...
With `tmux-resurrect` and `tmux-continuum` plugins, my sessions survive reboots. I close my laptop, open it a week later, and everything is exactly where I left it—same windows, same panes, same directories. That's the kind of workflow that compounds over time.
...
The `detach-on-destroy off` is clutch—when I close a session, instead of exiting tmux entirely, it switches to the next session. No more accidentally closing my entire tmux server.
...
I'm obsessed with it. The color palette is perfect—not too vibrant, not too washed out. The mauve, lavender, and pink accents pop without being distracting. And the fact that it has official ports for almost every tool I use means I can have a consistent theme everywhere.
...
**Next in this series**: [My Neovim Setup with 37 Plugins](/blog/neovim-37-plugins) - how I ditched VS Code and built a blazing-fast editor that actually sparks joy.
